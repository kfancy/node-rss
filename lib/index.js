...
function wrap_cdata(str) {
	if (str && typeof(str) == 'string' && (str.indexOf('&') > -1 || str.indexOf('<') > -1 || str.indexOf('>') > -1)) {
		return { _cdata: str };
	}
	return str;
}
...
function generateXML (data){
    ...
    data.items.forEach(function(item) {
        ...
        ifTruePush(item.url, item_values, { link: wrap_cdata(item.url) });
        ...
        ifTruePush((item.source && src.name && src.url), item_values, { source: [ { _attr: { url: src.url } }, src.name ] })
        ...
        if( item.enclosure && item.enclosure.url) {
            if( item.enclosure.file ) {
                ...
                var mLook = mime.lookup(item.enclosure.file);
                if (mLook && mLook !== 'false') {
                        PUSH.enclosure._attr.type = mLook;
                } else {
                	PUSH.enclosure._attr.type = 'application/octet-stream';
                }
                item_values.push(PUSH);
            } else {
                var PUSH = {
                    enclosure : {
                        _attr : {
                ...
                var mLook = mime.lookup(item.enclosure.url);
                if (mLook && mLook !== 'false') {
                        PUSH.enclosure._attr.type = mLook;
                } else if (item.enclosure.mimeType) {
                	PUSH.enclosure._attr.type = item.enclosure.mimeType;
                } else {
                	PUSH.enclosure._attr.type = 'application/octet-stream';
                }
                item_values.push(PUSH);
